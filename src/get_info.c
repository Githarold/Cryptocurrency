...
void make_url(void)
{
    strcpy(url, URL);                           // ticker 및 url을 가공하기 전으로 초기화한다.
    strcpy(ticker, "");

    SDL_SetWindowSize(app.window, 290, 50);     // 사용자에게 입력받을 화면을 띄우기 위해 크기를 작게 조정한다.

    SDL_StartTextInput();                       // 티커를 입력받기 위해 키보드입력을 받기 시작한다.
    SDL_bool done = SDL_FALSE;                  // 티커 입력이 끝났음을 알기 위해 SDL_bool 타입의 done 변수를 선언한다.
    while (!done)
    {
        ClearWindow();                          // 티커 입력화면을 띄운다
    ...
    strcat(url, ticker);                        //
    strcat(url, URL_OPTION);                    // 입력받은 티커를 이용해 JSON파일의 이름과 url 주소를 가공한다.
    strcat(ticker, JSON);                       //
...
void GetBoardInfo(void)
{
    ...
    {                                                   // 기본 잔고가 천만원으로 주어지기 때문에 수익금은 기본적으로 만원 단위가 넘어감
